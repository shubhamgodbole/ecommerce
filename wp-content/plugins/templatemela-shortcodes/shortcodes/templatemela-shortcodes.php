		...
		if(!empty($image)):
		 ...
		 if(empty($image)):
			...
			if($term){
                ...
                $output .='<div class="category_description" style="'.$cmsdescription_color.'">'.$term_category_description.'</div>';
...
// Display the submit button.
	...
	function shortcode_portfolio($atts, $content = null, $code) {
		...
		while(have_posts()) {
			...
			$output .= '<div class="links">';
			$output .= '<a href="'.$image.'" title="Click to view Full Image" class="icon mustang-gallery"><i class="fa fa-plus"></i></a>';
			$output .= '<a href="'.get_permalink().'"  title="'.esc_html__('Click to view read more', 'wallcart').'" class="icon"><i class="fa fa-link"></i></a>';	
			$output .= '</div></div>';
			$output .= '</div>';
